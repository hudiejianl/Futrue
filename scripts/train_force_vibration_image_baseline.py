...
from src.datasets.force_vibration_image_dataset import ForceVibrationImageDataset
from src.models.force_vibration_image_model import ForceVibrationImageModel


def parse_args():
    parser = argparse.ArgumentParser(description="Train force + vibration + image baseline.")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--vibration-feature-file",
        type=Path,
        default=Path(r"D:\my\Future\toolwear_multimodal\data\processed\index\vibration_cycle_features_train_val.csv"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(r"D:\my\Future\toolwear_multimodal\outputs\force_vibration_image_baseline"),
    )
...
def main():
    ...
    val_ds = ForceVibrationImageDataset(
        str(val_file),
        str(force_feature_file),
        str(args.vibration_feature_file),
        train=False,
    ...
    model = ForceVibrationImageModel(feature_input_dim=len(train_ds.feature_cols)).to(device)
