...
from src.datasets.force_vibration_feature_dataset import ForceVibrationFeatureDataset
from src.models.force_mlp import ForceMLP


def parse_args():
    parser = argparse.ArgumentParser(description="Train force + vibration baseline.")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(r"D:\my\Future\toolwear_multimodal\outputs\force_vibration_baseline"),
    )
...
def main():
    ...
    vib_feature_file = args.vibration_feature_file or (args.data_root / "index" / "vibration_cycle_features.csv")
    train_file = args.data_root / "splits" / "train.csv"
    val_file = args.data_root / "splits" / "val.csv"

    train_ds = ForceVibrationFeatureDataset(str(train_file), str(force_feature_file), str(vib_feature_file))
    feature_mean, feature_std = train_ds.compute_feature_stats()
    train_ds = ForceVibrationFeatureDataset(
        str(train_file),
        str(force_feature_file),
        str(vib_feature_file),
        feature_mean=feature_mean,
        feature_std=feature_std,
    )
    val_ds = ForceVibrationFeatureDataset(
        str(val_file),
        str(force_feature_file),
        str(vib_feature_file),
        feature_mean=feature_mean,
