...
from src.datasets.force_image_dataset import ForceImageDataset
from src.models.force_image_model import ForceImageModel


def parse_args():
    parser = argparse.ArgumentParser(description="Train force + image baseline.")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(r"D:\my\Future\toolwear_multimodal\outputs\force_image_baseline"),
    )
...
def evaluate(model, loader, device, reg_weight, cls_weight):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            total_loss += loss.item() * force_x.size(0)
...
def main():
    ...
    train_ds_raw = ForceImageDataset(str(train_file), str(feature_file), train=True)
    feature_mean, feature_std = train_ds_raw.compute_feature_stats()
    train_ds = ForceImageDataset(
        str(train_file),
        str(feature_file),
        train=True,
    ...
    model = ForceImageModel(force_input_dim=len(train_ds.feature_cols)).to(device)
